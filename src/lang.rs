...
mod parser {
    ...
    fn parse(iter: impl Iterator<Item = Token>) -> Result<Vec<Syntax>, String> {
...
mod interpreter {
    ...
    fn interpret(stmt: &Syntax, state: &mut State) {
        match stmt {
            ...
            Syntax::Write(_) => todo!(), // TODO (here is work todo)
            Syntax::Read(_) => todo!(),
            Syntax::Loop(_) => todo!(),
        }
    }
    ...
    fn run(program: Vec<Syntax>) -> State {
        let mut s = State::new();
